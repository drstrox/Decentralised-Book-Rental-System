...
const BookCard: React.FC<BookCardProps> = ({
  ...
  const calculateRentalDuration = () => {
    ...
    const daysRented = Math.ceil(secondsRented / 86400);
  ...
  const currentRentalCost = daysRented > 0 ? (parseFloat(dailyPrice) * daysRented).toFixed(4) : dailyPrice;
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden transform transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
      <div className="p-6">
        ...
        <p className="text-gray-600 mb-4">Owner: {shortenAddress(owner)}</p>
